...
const DEFAULT_AGENCY: AgencyDetails = {
  ownerName: 'Cycertron Karki',
  agencyName: 'Estatly Premium Realty',
  agencyDescription:
    'A premier real estate agency specialising in residential and commercial properties across Kathmandu Valley, offering expert market analysis, property valuations, and end-to-end deal management.',
  agencyLocation: 'Durbar Marg, Kathmandu, Nepal',
  agencyImages: [
    'https://images.unsplash.com/photo-1560518883-ce09059eeffa?w=800',
    'https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=800',
  ],
  agencyType: 'Mixed (Residential & Commercial)',
  verified: false,
  VerificationDocuments: ['https://example.com/docs/agency-license.pdf'],
  contactNumber: '9812345678',
  isactive: true,
  specializations: ['Residential Sales', 'Commercial Leasing', 'Property Management', 'Investment Consulting', 'Land Deals'],
  ...
  isFeatured: true,
  staff: [
    { id: '1', name: 'Sita Rana',    position: 'Senior Agent',       email: 'sita@estatly.com',  phone: '9812345601', joinDate: '2023-01-15', status: 'active'   },
    { id: '2', name: 'Anil Sharma',  position: 'Sales Manager',       email: 'anil@estatly.com',  phone: '9812345602', joinDate: '2023-06-20', status: 'active'   },
    { id: '3', name: 'Nisha Tamang', position: 'Property Consultant', email: 'nisha@estatly.com', phone: '9812345603', joinDate: '2023-03-10', status: 'inactive' },
  ],
};
...
const AgencySettingsPage: React.FC = () => {
  ...
  const [agencyDetails, setAgencyDetails] = useState<AgencyDetails>(() => {
    try {
      const saved = localStorage.getItem('agencyDetails');
      return saved ? JSON.parse(saved) : DEFAULT_AGENCY;
    } catch {
      return DEFAULT_AGENCY;
    }
  });
  const [formData, setFormData]               = useState<AgencyDetails>(agencyDetails);
  const [specializationsInput, setSpecializationsInput] = useState(agencyDetails.specializations.join(', '));
  const [imageInput, setImageInput]           = useState(agencyDetails.agencyImages.join('\n'));
  const [staffList, setStaffList]             = useState<Staff[]>(agencyDetails.staff);
  ...
  useEffect(() => {
    const fetchAgencyData = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:3001/api/v1/agencies/my-agency', {
          headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        });
        if (response.data?.data) {
          const d = response.data.data;
          const formatted: AgencyDetails = {
            ownerName: d.ownerName || '', agencyName: d.agencyName || '',
            agencyDescription: d.agencyDescription || '', agencyLocation: d.agencyLocation || '',
            agencyImages: d.agencyImages || [], agencyType: d.agencyType || '',
            ...
            specializations: d.specializations || [], officeHoursOpen: d.officeHoursOpen || '09:00',
            officeHoursClose: d.officeHoursClose || '18:00', commissionRate: d.commissionRate || 0,
            rating: d.rating || 0, numberOfReviews: d.numberOfReviews || 0,
            isFeatured: d.isFeatured || false, staff: d.staff || DEFAULT_AGENCY.staff,
          };
          setAgencyDetails(formatted); setFormData(formatted);
          setSpecializationsInput(formatted.specializations.join(', '));
          setImageInput(formatted.agencyImages.join('\n'));
          setStaffList(formatted.staff);
          localStorage.setItem('agencyDetails', JSON.stringify(formatted));
        }
      } catch (err: any) {
        setError(err.response?.data?.message || err.message || 'Failed to load agency data');
        const saved = localStorage.getItem('agencyDetails');
        if (saved) {
          const parsed: AgencyDetails = JSON.parse(saved);
          setAgencyDetails(parsed); setFormData(parsed);
          setSpecializationsInput(parsed.specializations.join(', '));
          setImageInput(parsed.agencyImages.join('\n'));
  ...
  const handleSave = () => {
    const updated: AgencyDetails = {
      ...formData,
      specializations: specializationsInput.split(',').map((f) => f.trim()).filter(Boolean),
      agencyImages:    imageInput.split('\n').map((i) => i.trim()).filter(Boolean),
      staff: staffList,
    };
    setAgencyDetails(updated);
    localStorage.setItem('agencyDetails', JSON.stringify(updated));
    setIsEditing(false);
  };

  const handleCancel = () => {
    setFormData(agencyDetails);
    setSpecializationsInput(agencyDetails.specializations.join(', '));
    setImageInput(agencyDetails.agencyImages.join('\n'));
    setIsEditing(false);
  };
  ...
  const D = isEditing ? formData : agencyDetails;
      ...
      <header className="relative bg-[#3D1F12] overflow-hidden px-10 pt-10 pb-8 flex items-end justify-between gap-6 flex-wrap">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-2.5 mb-2.5">
            ...
            <span className="text-[10px] font-bold tracking-[3px] uppercase text-amber-300/80">Agency Management</span>
            <span className="h-px w-6 bg-amber-400/60" />
          </div>
          <h1 className="font-serif text-5xl font-semibold text-[#FFF8F0] leading-tight tracking-tight">
            <span className="text-amber-300 italic">Agency</span> Settings
          </h1>
          <p className="mt-2 text-sm text-[#FFF8F0]/40 font-light tracking-wide">
            {D.agencyName} · {D.agencyLocation}
          </p>
        </div>
      ...
      <div className="px-10 pt-8 pb-16 flex flex-col gap-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <Card>
            ...
            <Field label="Agency Name"      name="agencyName"   value={D.agencyName}   onChange={handleInputChange} />
            <Field label="Agency Type"      name="agencyType"   value={D.agencyType}   onChange={handleInputChange} />
            <Field label="Contact Number"   name="contactNumber" type="tel" value={D.contactNumber} onChange={handleInputChange} />
          </Card>

          <Card>
            <CardTitle icon="📍">Location & Commission</CardTitle>
            <Field label="Location" name="agencyLocation" value={D.agencyLocation} onChange={handleInputChange} />

            <div className="mb-5">
              <label className={labelCls}>Commission Rate (%)</label>
              {isEditing ? (
                <input className={inputCls} type="number" name="commissionRate" value={formData.commissionRate} onChange={handleInputChange} />
              ) : (
                <p className="font-serif text-2xl font-bold text-amber-600 mt-0.5">
                  {D.commissionRate}% per deal
                </p>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className={labelCls}>Office Opens</label>
                {isEditing
                  ? <input className={inputCls} type="time" name="officeHoursOpen" value={formData.officeHoursOpen} onChange={handleInputChange} />
                  : <p className="text-sm font-medium text-stone-700 mt-0.5">{D.officeHoursOpen}</p>}
              </div>
              <div>
                <label className={labelCls}>Office Closes</label>
                {isEditing
                  ? <input className={inputCls} type="time" name="officeHoursClose" value={formData.officeHoursClose} onChange={handleInputChange} />
                  : <p className="text-sm font-medium text-stone-700 mt-0.5">{D.officeHoursClose}</p>}
              </div>
            </div>
          </Card>
        </div>
        ...
        {/* Row 2: Description */}
        <Card>
          <CardTitle icon="✍️">Agency Description</CardTitle>
          {isEditing ? (
            <textarea
            ...
            <p className="text-sm font-medium text-stone-600 leading-[1.8]">{D.agencyDescription}</p>
          )}
        </Card>

        {/* Row 3: Specializations + Rating */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">

          <Card>
            <CardTitle icon="🏢">Specializations</CardTitle>
            {isEditing ? (
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g. Residential Sales, Commercial Leasing, Land Deals…"
                />
              </div>
            ) : (
              <div className="flex flex-wrap gap-2">
                {D.specializations.map((f, i) => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <Card>
            ...
            <div className="flex flex-col gap-3">
              {(
                [
                  { name: 'isactive',   label: 'Agency Active', value: formData.isactive,   display: D.isactive   },
          ...
          <Card>
            <CardTitle icon="🖼">Agency Photos</CardTitle>
            {isEditing ? (
              <div>
            ...
            ) : D.agencyImages.length > 0 ? (
              <div className="flex flex-col gap-2">
                {D.agencyImages.map((img, i) => (
                  <a
                    key={i}
                    href={img}
                    target="_blank"
