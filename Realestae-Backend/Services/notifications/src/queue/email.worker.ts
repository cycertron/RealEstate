...
const registerEmailWorker = new Worker<RegisterEmailJobData>(
  ...
  async (job: Job<RegisterEmailJobData>) => {
    try {
      ...
      await sendEmail(
        to,
        "Welcome to Realestae - Your Journey Begins Here!",
        getWelcomeLoginTemplate({
          user_name: userName,
          app_link: process.env.APP_LINK || "https://cycertron9.com.np",
...
const otpEmailWorker = new Worker<OTPEmailJobData>(
  ...
  async (job: Job<OTPEmailJobData>) => {
    try {
      ...
      const appLink = process.env.APP_LINK || "https://cycertron9.com.np";
      await sendEmail(email, "Your OTP Code for Realestae", getTwoFactorAuthTemplate({
        user_name: Name,
